...
public:
    ...
    virtual Status DeallocateDiskSpace(uint64_t offset, size_t len) { // 释放指定磁盘空间
...
public:
    ...
    virtual Status NewWritableFile(const std::string& fname, WritableFile** result) {
        ...
        FILE* f = fopen(fname.c_str(),
                        "w"); // w写入方式打开，将文件指针指向文件头并将文件大小截为零。如果文件不存在则尝试创建之。
        if (f == NULL) {
            *result = NULL;
            s = PosixError(fname, errno);
        } else {
    ...
    virtual bool FileExists(const std::string& fname) { return access(fname.c_str(), F_OK) == 0; }
    ...
    virtual Status GetTestDirectory(std::string* result) {
        ...
        if (env && env[0] != '\0') {
            *result = env;
            ...
            snprintf(buf, sizeof(buf), "/tmp/leveldbtest-%d", 11);
            *result = buf;
        }
