...
void VersionEdit::Clear() {
    ...
    has_sort_head_info_ = false;
    has_tail_info_ = false;
    deleted_files_.clear();
    new_files_.clear();
    has_full_gc_file_ = false;
}
...
void VersionEdit::EncodeTo(std::string* dst) const {
    ...
    if (has_sort_head_info_) {
        PutVarint32(dst, kSortHead);
        dst->append(sort_head_info_, sizeof(sort_head_info_));
    }
    if (has_tail_info_) {
    ...
    if (has_full_gc_file_) {
        PutVarint32(dst, kGCFile);
        PutVarint64(dst, full_gc_file_);
    }
...
Status VersionEdit::DecodeFrom(const Slice& src) {
    ...
    while (msg == NULL && GetVarint32(&input, &tag)) {
        switch (tag) {
            ...
            case kSortHead:
                if (memcpy(sort_head_info_, input.data(), sizeof(sort_head_info_))) {
                    input.remove_prefix(sizeof(sort_head_info_));
            ...
            case kGCFile:
                if (GetVarint64(&input, &full_gc_file_)) {
