...
static int TargetFileSize(const Options* options) { return options->max_file_size; }
...
public:
    ...
    virtual uint64_t fileNumber() {
        if (index_ == flist_->size()) index_ = 0;
        return (*flist_)[index_]->number;
    }
...
static void SaveValue(void* arg, const Slice& ikey, const Slice& v) {
    ...
    if (!ParseInternalKey(ikey, &parsed_key)) {
        ...
        // if (s->ucmp->Compare(parsed_key.user_key, s->user_key) == 0 && s->seq < parsed_key.sequence) {
...
Status Version::Get(const ReadOptions& options, const LookupKey& k, std::string* value, GetStats* stats,
                    std::tr1::unordered_set<uint64_t>* full_gc_file, std::vector<uint64_t>* visited_file) {
    ...
    for (int level = 0; level < config::kNumLevels; level++) {
        ...
        for (uint32_t i = 0; i < num_files; ++i) {
            ...
            if (visited_file != NULL) {
                visited_file->push_back(f->number);
            }
            Saver saver;
            ...
            if (!s.ok()) {
                std::cout << "cannot get sstable  1\n";
                return s;
            }
            if (saver.state == kCorrupt) {
                s = Status::Corruption("corrupted key for ", user_key);
                return s;
            } else if (saver.state == kDeleted) {
                s = Status::NotFound(Slice());
                return s;
            } else if (saver.state == kNotFound) {
                if (full_gc_file && full_gc_file->count(f->number) > 0) {
                    if (has_found) {
                        *value = tmp_value;
                        return Status::OK();
                    }
        ...
        if (visited_file != NULL) {
            visited_file->push_back(level);
        }
        if (has_found) {
            if (visited_file != NULL) {
                visited_file->push_back(10);
                if (full_gc_file) {
                    for (auto iter = full_gc_file->begin(); iter != full_gc_file->end(); iter++) {
                        visited_file->push_back(*iter);
                    }
                }
            }
            *value = tmp_value;
...
Status VersionSet::Recover(bool* save_manifest) {
    ...
    if (!s.ok()) {
        if (s.IsNotFound()) {
            return Status::Corruption("CURRENT points to a non-existent file1", s.ToString());
        }
        return s;
    }
...
Status VersionSet::Recover(bool* save_manifest, uint64_t& vlog_number, uint64_t& head_pos, uint64_t& sort_vlog_number,
                           uint64_t& sort_head_pos, std::string& vloginfo, uint64_t& clean_vlog_number,
                           uint64_t& clean_pos) {
    struct LogReporter : public log::Reader::Reporter {
        Status* status;
        virtual void Corruption(size_t bytes, const Status& s) {
    ...
    if (!s.ok()) {
        if (s.IsNotFound()) {
            return Status::Corruption("CURRENT points to a non-existent file2", s.ToString());
        }
        return s;
    }
...

    {
        ...
        while (reader.ReadRecord(&record, &scratch) && s.ok()) {
            ...
            if (edit.has_full_gc_file_) {
                full_gc_set_.insert(edit.full_gc_file_);
            }
            ...
            if (edit.has_sort_head_info_) {
                uint64_t code = DecodeFixed64(edit.sort_head_info_);
                sort_vlog_number = code & 0xffffff;
                sort_head_pos = code >> 24;
                // std::cout<<"3 recover sort vlog="<<sort_vlog_number<<" head="<<sort_head_pos<<"\n";
            }
    ...
    if (s.ok()) {
        ...
        last_sequence_ = last_sequence;
        log_number_ = log_number;
        sort_log_number_ = sort_vlog_number;
        prev_log_number_ = prev_log_number;
...
void VersionSet::MarkSortVlogNumberUsed(uint64_t number) {
    if (sort_log_number_ < number) {
        sort_log_number_ = number;
    }
}
...
void VersionSet::InitialFileMap() {
    for (int i = 0; i < config::kNumLevels; i++) {
        for (int j = 0; j < current_->files_[i].size(); j++) {
            file_maps_[current_->files_[i][j]->number] = i;
        }
    }
}
...
bool VersionSet::NeedsCompaction() {
    return !resort_set_.empty() || current_->files_[0].size() >= config::kL0_CompactionTrigger ||
           current_->compaction_score_ >= 1 || current_->file_to_compact_ != NULL;
}
...
void VersionSet::PrintFileSize() {
    for (size_t i = 0; i < config::kNumLevels; i++) {
        if (current_->files_[i].size() == 0) continue;
        std::cout << "level " << i << "= ";
        for (size_t j = 0; j < current_->files_[i].size(); j++) {
            std::cout << current_->files_[i][j]->number << " ";
        }
        std::cout << "\n";
    }
    std::cout << "full_gc_set_= ";
    for (auto iter = full_gc_set_.begin(); iter != full_gc_set_.end(); iter++) {
        std::cout << *iter << " ";
    }
    std::cout << "\n";
}
...
Iterator* VersionSet::MakeInputIterator(Compaction* c, Iterator* gc_mem) {
    // std::cout << "start MakeInputIterator\n";
    ReadOptions options;
    ...
    const int space = (c->level() == 0 ? c->inputs_[0].size() + 1 : 2) + (gc_mem != NULL ? 1 : 0);
    Iterator** list = new Iterator*[space];
    int num = 0;
    // std::cout << "finish MakeInputIterator 1\n";
    if (gc_mem != NULL) {
        list[num++] = gc_mem;
        list[num - 1]->SeekToFirst();
    }
    ...
    // std::cout << "finish MakeInputIterator 2\n";
    delete[] list;
    return result;
}
...
void VersionSet::Finalize(Version* v) {
    int best_level = 0;
    double best_score = v->files_[0].size() / static_cast<double>(config::kL0_CompactionTrigger);
    for (int level = 1; level < config::kNumLevels - 1; level++) {
        double score = static_cast<double>(TotalFileSize(v->files_[level])) / MaxBytesForLevel(options_, level) *
                       (1 + 0.2 * level);
        if (score > best_score) {
            best_level = level;
            best_score = score;
...
void VersionSet::SupplementCompaction(Compaction* c) {
    c->ReleaseInputs();
    delete c;
    c = new Compaction(options_, 0);
    for (size_t i = 0; i < current_->files_[0].size(); i++) {
        if (full_gc_set_.count(current_->files_[0][i]->number) > 0) {
    ...
    SetupOtherInputs(c);
    // std::cout << "SupplementCompaction " << c->inputs_[0].size() << " " << c->inputs_[1].size() << "     "
    //           << current_->files_[0].size() << " " << current_->files_[1].size() << " " << current_->files_[2].size()
    //           << " " << current_->files_[3].size() << " " << current_->files_[4].size() << "\n";
}

Compaction* VersionSet::PickCompaction() {
    // std::cout << "start PickCompaction\n";
    Compaction* c;
    int level;
    if (!resort_set_.empty()) {
        int min_level = config::kNumLevels;
        for (auto iter = resort_set_.begin(); iter != resort_set_.end(); iter++) {
            if (min_level > file_maps_[*iter]) {
                min_level = file_maps_[*iter];
            }
        }
        level = min_level;
        ...
        for (size_t i = 0; i < current_->files_[level].size(); i++) {
            if (resort_set_.count(current_->files_[level][i]->number)) {
                c->inputs_[0].push_back(current_->files_[level][i]);
                break;
            }
        }
    } else if (current_->compaction_score_ >= 1) {
        if (current_->file_to_compact_ != NULL && current_->compaction_level_ == current_->file_to_compact_level_) {
            ...
            c = new Compaction(options_, level);
            if (level == 0) {
                uint64_t largest_numb = 0;
                FileMetaData* f;
                for (size_t i = 0; i < current_->files_[level].size(); i++) {
                    if (largest_numb < current_->files_[level][i]->number) {
                        largest_numb = current_->files_[level][i]->number;
    ...
    } else if (current_->file_to_compact_ != NULL) {
        level = current_->file_to_compact_level_;
        c = new Compaction(options_, level);
        c->inputs_[0].push_back(current_->file_to_compact_);
    } else {
    ...
    SetupOtherInputs(c);
    // std::cout << "PickCompaction " << level << " " << c->inputs_[0].size() << " " << c->inputs_[1].size() << "     "
    //           << current_->files_[0].size() << " " << current_->files_[1].size() << " " << current_->files_[2].size()
    //           << " " << current_->files_[3].size() << " " << current_->files_[4].size() << "\n";

    for (size_t i = 0; current_->file_to_compact_ && i < 2; i++) {
        for (size_t j = 0; j < c->inputs_[i].size(); j++) {
            if (current_->file_to_compact_->number == c->inputs_[i][j]->number) {
