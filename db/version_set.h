...
extern uint64_t MaxFileSizeForLevel(const Options* options, int level);
...
class Version {
public:
    void SetCompactionScoreAndLevel(double compaction_score, int compaction_level);
    ...
    Status Get(const ReadOptions&, const LookupKey& key, std::string* val, GetStats* stats,
               std::tr1::unordered_set<uint64_t>* full_gc_file = NULL, std::vector<uint64_t>* visited_file = NULL);
...
public:
    ...
    Status Recover(bool* save_manifest, uint64_t& log_number, uint64_t& head_pos, uint64_t& sort_vlog_number,
                   uint64_t& sort_head_pos, std::string& vloginfo, uint64_t& clean_vlog_number, uint64_t& clean_pos);
    ...
    uint64_t NewSortVlogNumber() { return ++sort_log_number_; }
    ...
    void ReuseSortVlogNumber(uint64_t file_number) {
        if (sort_log_number_ == file_number) {
            sort_log_number_ = file_number - 1;
        }
    }
    ...
    void MarkSortVlogNumberUsed(uint64_t number);

    // Return the current log file number.
    uint64_t LogNumber() const { return log_number_; }
    uint64_t SortLogNumber() const { return sort_log_number_; }
    ...
    void SupplementCompaction(Compaction* c);
    ...
    Iterator* MakeInputIterator(Compaction* c, Iterator* gc_mem = NULL);
    ...
    std::tr1::unordered_set<uint64_t> resort_set_;
    std::tr1::unordered_set<uint64_t> full_gc_set_;
    std::tr1::unordered_map<uint64_t, int> file_maps_;
    port::Mutex mutex_;
...
extern int64_t TotalFileSize(const std::vector<FileMetaData*>& files);
