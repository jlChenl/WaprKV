...
public:
    explicit MemTableIterator(MemTable::Table* table) : iter_(table) { file_numb_ = 1; }
    ...
    virtual uint64_t fileNumber() { return file_numb_; }
...
bool MemTable::Get(const LookupKey& key, std::string* value, Status* s, uint64_t* seq) {
