...
public:
    VlogTest()
        ...
        , reader_(new VReader(ps_, &report_, true /*checksum*/, 0 /*initial_offset*/)) {
    ...
    void CheckReadRecord() {
        ...
        for (; expected_record_offset < num_initial_offset_records_; ++expected_record_offset) {
            ...
            ASSERT_EQ((char)('a' + expected_record_offset),
                      buf[initial_offset_record_sizes_[expected_record_offset] - 1]);
        }
...
int VlogTest::num_initial_offset_records_ = sizeof(VlogTest::initial_offset_record_sizes_) / sizeof(size_t);

TEST(VlogTest, Empty) { ASSERT_EQ("EOF", Read()); }
...
TEST(VlogTest, ReadStart) { CheckInitialOffsetRecord(0, 0); }

TEST(VlogTest, ReadSecondOneOff) { CheckInitialOffsetRecord(kBlockSize, 1); }

TEST(VlogTest, Read) { CheckReadRecord(); }
