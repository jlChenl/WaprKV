...
VReader::VReader(SequentialFile* file, Reporter* reporter, bool checksum, uint64_t initial_offset)
    : file_(file)
    , reporter_(reporter)
    , checksum_(checksum)
    , backing_store_(new char[kBlockSize])
...
bool VReader::ReadRecord(Slice* record, std::string* scratch, int& head_size) { // 日志回放的时候是单线程
    scratch->clear();
    record->clear();
    ...
    uint32_t expected_crc =
        crc32c::Unmask(DecodeFixed32(buffer_.data())); // 早一点解析出crc,因为后面可能buffer_.data内容会变
...
void VReader::ReportCorruption(uint64_t bytes, const char* reason) { ReportDrop(bytes, Status::Corruption(reason)); }
...
bool VReader::DeallocateDiskSpace(uint64_t offset, size_t len) { return file_->DeallocateDiskSpace(offset, len).ok(); }
